...
# ----------------------
  #rspec spec/models/activerecord_obstacle_course_spec.rb:43
  ...
  it '2. finds order id of smallest order' do
    ...
    order_id = Order.order(:amount).limit(1)[0].id
    # ------------------------------------------------------------
...
# ========================
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    # names = Item.joins(:orders).where(order_items: {order_id: order.id}).order(_name).pluck(:name)
    #names = Order.last.items.order("name REGEXP ?",("|x| x[/\d+/].to_i")).pluck(:name)
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    items_for_user_3_third_order = @user_3.orders[2].items.order(:id).pluck(:name)
    # ------------------------------------------------------------
...
# ========================
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    #  Solution goes here
    orders = @user_2.orders.joins(:items)
                  .where(order_items: {item_id: @item_4.id})
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    # Solution goes here
    ordered_items = Item.joins(:orders).group(:id)
    # ---------------------------------------------------------------
...
# ========================
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.joins(:orders).group(:id).pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select('users.name, count(orders.id) as total_order_count').group(:name).order(:name)
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).select('users.name, count(items.id) as total_item_count').group(:name).order(name: :desc)
    # ---------------------------------------------------------------
